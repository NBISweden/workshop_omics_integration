...
<img border="0" src="img/conda_illustration.png" width="600">
...
You should start by installing Conda. We suggest installing either Miniconda3 (NOT Anaconda). After [installing Conda](https://docs.conda.io/projects/conda/en/latest/user-guide/install/index.html), download the conda file and put it in your working folder.
...
In the [labs](labs.md) page, we provide links for each environment for each hands-on session. To download an environment file such as `env-merged_nets.yaml` using the command on Terminal:
...
typing `rstudio` or jupyter with `jupyter-notebook`. Here it is important to add the `&` symbol in the end to be able to use the command line at the same time if needed. You can open other files from Rstudio later as well.

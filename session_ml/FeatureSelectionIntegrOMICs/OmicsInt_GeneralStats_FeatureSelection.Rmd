---
title: "Omics Integration course: <br > General Advanced Mathematical Statistics: <br> Feature Selection"
...
knitr::opts_knit$set(root.dir="/Users/rui.benfeitas/OneDrive/GitHub/workshop_omics_integration/session_ml/FeatureSelectionIntegrOMICs/")
```
...
length(Y)
```
The data set used here includes 99 Males and 58 Females, it is not perfectly balanced but still not too bad. To visualize the samples, let us display a PCA plot of the 157 samples. We will use the package `mixOmics` to do this. 

```{r eval=FALSE}
##Download the file "mixOmics_6.3.1.tar.gz"
##Install it in R using:
install.packages("mixOmics_6.3.1.tar.gz", repos = NULL, type="source", dependencies = TRUE)

#If you get a "missing dependencie" error, run the following:
install.packages(c("corpcor","ellipse","rgl","rARPACK","tidyr"))
```
